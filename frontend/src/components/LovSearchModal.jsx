...
export default function LovSearchModal({ onClose }) {
  ...
  useEffect(() => {
    fetch(`${API}/lovs/attributes`).then(r => r.json()).then(setAttributes).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    const onKey = e => {
      if (e.key === "Escape") {
        if (selected) { setSelected(null); setValQuery(""); setValues([]); }
        else onClose();
      }
    };
  ...
  const filteredAttrs = useMemo(() => {
    ...
    return q ? attributes.filter(a => a.toLowerCase().includes(q)) : attributes;
  }, [attributes, query]);

  const filteredValues = useMemo(() => {
    const q = valQuery.trim().toLowerCase();
    return q ? values.filter(v =>
      v.key?.toLowerCase().includes(q) || v.description?.toLowerCase().includes(q)
    ) : values;
  }, [values, valQuery]);

  const goBack = () => { setSelected(null); setValQuery(""); setValues([]); };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxHeight: "min(80vh, 680px)", background: "var(--modal-bg, white)" }}
      ...
      >
        ...
        <div className="bg-white dark:bg-slate-900 flex items-center gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
          ...
          <input
            ...
            onChange={e => selected ? setValQuery(e.target.value) : setQuery(e.target.value)}
            placeholder={selected ? `Filtrer dans ${selected}…` : "Rechercher un attribut LOV…"}
        ...
        {selected && (
          <div className="bg-slate-50 dark:bg-slate-800/50 px-5 py-2 flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
            <span className="text-xs text-slate-400 dark:text-slate-500">Attribut</span>
            <ChevronRight className="h-3 w-3 text-slate-300 dark:text-slate-600" />
            <span className="text-xs font-semibold text-brand-600 dark:text-brand-400">{selected}</span>
            <span className="ml-auto text-xs text-slate-400">
              {loading ? "…" : `${filteredValues.length} valeur${filteredValues.length > 1 ? "s" : ""}`}
            </span>
          </div>
        )}
          ...
          {!selected && (
            filteredAttrs.length === 0 ? (
              <div className="py-16 text-center">
                <p className="text-sm text-slate-400">Aucun attribut pour «&nbsp;<span className="text-slate-600 dark:text-slate-300">{query}</span>&nbsp;»</p>
              </div>
            ) : (
              <ul className="py-2">
          ...
          {selected && (
            loading ? (
              ...
              <div className="py-16 text-center text-sm text-slate-400">
                {valQuery ? `Aucun résultat pour «\u00a0${valQuery}\u00a0»` : "Aucune valeur."}
              </div>
            ) : (
              <table className="w-full">
                <thead className="sticky top-0 bg-slate-50/90 dark:bg-slate-800/90 backdrop-blur-sm border-b border-slate-100 dark:border-slate-700/50">
                  <tr>
                    <th className="px-5 py-2.5 text-left text-[10px] font-semibold uppercase tracking-widest text-slate-400 w-44">Clé</th>
                    <th className="px-5 py-2.5 text-left text-[10px] font-semibold uppercase tracking-widest text-slate-400">Description</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/50">
                  {filteredValues.map((v, i) => (
                    <tr key={i} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-5 py-3 text-sm text-slate-600 dark:text-slate-300">{v.description}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 px-5 py-2.5 flex-shrink-0 flex items-center justify-between">
          <span className="text-[11px] text-slate-400">
            {selected
              ? <><span className="text-slate-500 dark:text-slate-400 font-medium">{filteredValues.length}</span> valeur{filteredValues.length > 1 ? "s" : ""}</>
              : <><span className="text-slate-500 dark:text-slate-400 font-medium">{filteredAttrs.length}</span> attribut{filteredAttrs.length > 1 ? "s" : ""}</>
            }
          </span>
          <div className="flex items-center gap-3 text-[11px] text-slate-400">
            {selected
              ? <span><kbd className="px-1.5 py-px font-mono rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">Esc</kbd> pour revenir</span>
              : <span><kbd className="px-1.5 py-px font-mono rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700">↵</kbd> pour ouvrir</span>
            }
          </div>
        </div>
      </div>
