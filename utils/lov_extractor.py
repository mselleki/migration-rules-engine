...
if __name__ == "__main__":
    ...
    lovs_path = root / "LOVs.xlsx"

    if not lovs_path.exists():
        raise SystemExit(f"LOVs workbook not found at: {lovs_path}")

    flat_df = extract_lovs_from_excel(str(lovs_path))
    output_path = root / "input" / "lovs_flat.csv"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    flat_df.to_csv(output_path, index=False)
    print(f"Wrote {len(flat_df)} LOV rows to {output_path}")
